...
export class LocalObjectStore {
  ...
  async putFile(sourcePath, objectKey) { const destination = join(this.root, cleanKey(objectKey)); await mkdir(dirname(destination), { recursive: true }); await rename(sourcePath, destination); return { objectKey: cleanKey(objectKey) } }
